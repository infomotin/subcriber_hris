...
@section('title', isset($type) ? 'Edit Movement Type' : 'Add Movement Type')
...
<div class="page-title-box d-flex align-items-center justify-content-between mb-4">
    <div>
        ...
        <h4 style="font-family:'Poppins',sans-serif;font-weight:700;color:#0f172a;">
            <i class="bx bx-transfer-alt text-primary me-1.5 align-middle font-size-26"></i>{{ isset($type) ? 'Edit' : 'Add' }} Movement Type
        </h4>
    </div>
    ...
    <div class="col-lg-6">
        <div class="card border-0 shadow-sm" style="border-radius:14px;">
            <div class="card-body p-4">
                <form action="{{ isset($type) ? route('subscriber.hris.movement-types.update', $type) : route('subscriber.hris.movement-types.store') }}" method="POST">
                    @csrf
                    @if(isset($type)) @method('PUT') @endif

                    <div class="mb-3">
                        <label class="form-label fw-semibold text-slate-700">Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $type->name ?? '') }}" required placeholder="e.g. Short Leave">
                        @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold text-slate-700">Code <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('code') is-invalid @enderror" name="code" value="{{ old('code', $type->code ?? '') }}" required placeholder="e.g. SL" maxlength="20">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.5" min="0.5" max="24" class="form-control @error('max_hours') is-invalid @enderror" name="max_hours" value="{{ old('max_hours', $type->max_hours ?? '3') }}" required>
                        @error('max_hours') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    <div class="mb-3">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="requires_return" id="requiresReturn" value="1" {{ old('requires_return', $type->requires_return ?? 1) ? 'checked' : '' }}>
                            <label class="form-check-label fw-semibold text-slate-700" for="requiresReturn">Requires Return Time</label>
                        </div>
                        <small class="text-muted">Disable for Day Out type (no return needed)</small>
                    </div>

                    <div class="mb-3">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="is_active" id="isActive" value="1" {{ old('is_active', $type->is_active ?? 1) ? 'checked' : '' }}>
                    ...
                    <div class="text-end mt-4 pt-3 border-top">
                        ...
                        <button type="submit" class="btn btn-primary rounded-pill px-5" style="height:44px;">
                            <i class="bx bx-save me-1.5 align-middle font-size-18"></i> {{ isset($type) ? 'Update' : 'Save' }} Type
                        </button>
                    </div>
                </form>
            </div>
...
document.getElementById('durationType').addEventListener('change', function() {
    const rr = document.getElementById('requiresReturn');
    if (this.value === 'day_out') {
        rr.checked = false;
    } else {
        rr.checked = true;
    }
