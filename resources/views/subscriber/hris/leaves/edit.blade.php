    ...
    <div class="col-lg-6">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4">
                <form action="{{ route('subscriber.hris.leaves.update', $leave) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('employee_profile_id') is-invalid @enderror" id="employee_profile_id" name="employee_profile_id" required>
                            ...
                            @foreach($employees as $emp)
                                <option value="{{ $emp->id }}" {{ old('employee_profile_id', $leave->employee_profile_id) == $emp->id ? 'selected' : '' }}>{{ $emp->user->name ?? 'N/A' }} ({{ $emp->employee_id }})</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('leave_type_id') is-invalid @enderror" id="leave_type_id" name="leave_type_id" required>
                            ...
                            @foreach($leaveTypes as $type)
                                <option value="{{ $type->id }}" {{ old('leave_type_id', $leave->leave_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }} ({{ $type->days_per_year }} days/year)</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('reason') is-invalid @enderror" id="reason" name="reason" rows="3" required placeholder="Describe the reason for leave...">{{ old('reason', $leave->reason) }}</textarea>
                    ...
                    <div class="text-end mt-4">
                        <button type="submit" class="btn btn-primary px-4 rounded-pill">
                            <i class="bx bx-save me-1"></i> Update Application
                        </button>
                    </div>
                </form>
            </div>
